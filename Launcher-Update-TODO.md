...
- [x] **Laravel-Backend live** (`/var/www/minetrax`, DB vorher per `mysqldump` gesichert):
  ...
  - `friends:sync-accepted` — läuft jede Minute (Scheduler), schreibt akzeptierte Anfragen in `mmocore_playerdata.friends` (kein Autosave-Konflikt)
...
- [x] **Bugfix (21.07.2026): `/addfriend` fand echte, registrierte Spieler nicht.** Ursache: `Bukkit.getOfflinePlayer(name)` kennt nur Spieler, die genau auf `lobby-1` im lokalen Cache gelandet sind, nicht netzwerkweit. Neue Klasse `PlayerLookup.java` fragt stattdessen `lastloginapi_players` per JDBC direkt ab (read-only User `erzmark_readonly`, MySQL-Treiber shaded+relocated in der jar). Zweiter Bugfix direkt danach: `PlayerData.get(OfflinePlayer)` wirft bei echten offline Spielern `"Data is not loaded"` (Methoden-Signatur ist irreführend) — für offline Zielspieler wird jetzt gar kein MMOCore-`FriendRequest`-Objekt mehr gebaut (nicht nötig, die Anfrage läuft eh über Laravel), nur noch für online Zielspieler (native Chat-Annahme).
- [x] **Bugfix (21.07.2026): Annehmen im Launcher kam ingame nicht an.** Zwei Ursachen in `SyncAcceptedFriendRequests.php` behoben: (1) wartete auf **beide** Spieler gleichzeitig offline statt jede Seite unabhängig zu synchronisieren, sobald genau sie offline ist; (2) nutzte `lastloginapi_players` für die Online-Prüfung — dieselbe Tabelle, die im Launcher-Code (`config.rs`) schon als unzuverlässig dokumentiert ist, blieb im Test minutenlang fälschlich auf "online" stehen. Nutzt jetzt CloudNet-Rest direkt (`network-status.php`s Datenquelle, "netzwerkweit korrekt"). Live verifiziert: Freundschaft wurde korrekt geschrieben, sobald CloudNet den Spieler tatsächlich als offline meldete.
...
- [x] **Mobile App angebunden (22.07.2026):** `src/api/notifications.js`, `src/state/NotificationsContext.jsx`, `src/components/NotificationBell.jsx` (in `HomeHeader.jsx` eingebunden) + Freundschaftsanfrage-Karten in `FriendsScreen.jsx` — gleiches Format/Verhalten wie Launcher, per Babel-Parser syntaxgeprüft (kein Metro/Device in dieser Session verfügbar).
- [ ] **Noch zu tun:** echter End-to-End-Test im laufenden Tauri-Launcher und in der App (bisher nur der Backend-Teil bis Laravel live getestet)

### Teil 4: Freund entfernen + Live-Sync auch bei Online-Spielern ✅ Backend + Plugin + UI (22.07.2026)
- **Nutzerwunsch:** (1) Entfernen muss auch ingame unter `/friends` verschwinden. (2) Annehmen soll auch **sofort bei online Spielern** wirken, nicht erst nach Ausloggen (10-20s Toleranz wegen Server-Last).
- [x] **Neue Tabelle `friend_removals`** (Migration, Model) + **`POST /app-api/friends/remove`** (Sanctum) — legt eine Entfernung an, braucht keine Zustimmung der Gegenseite (anders als beim Hinzufügen).
- [x] **`SyncFriendRemovals`-Command** (Scheduler, jede Minute) — Gegenstück zu `SyncAcceptedFriendRequests`, entfernt aus `mmocore_playerdata.friends`, sobald die jeweilige Seite offline ist. CloudNet-Präsenzprüfung dafür in `App\Services\CloudNetPresenceService` ausgelagert (von beiden Sync-Commands genutzt, keine Duplikation).
- [x] **`GET /api/v1/social/pending-operations`** (Plugin-Secret) — liefert offene Annahmen (`adds`) und Entfernungen (`removes`).
- [x] **`PendingSyncWatcher`** (neue Plugin-Klasse, pollt alle 15s, `sync-poll-interval-ticks` in `config.yml`): wendet Änderungen für gerade **online** Spieler sofort über MMOCores eigene `PlayerData.addFriend`/`removeFriend`-API an (kein DB-Zugriff nötig — MMOCores eigener Autosave persistiert das von selbst, kein Konflikt mit dem Laravel-Fallback möglich, da beide Wege idempotent den aktuellen Stand zuerst prüfen). Löst damit genau den Fall "beide online" live, ohne auf Ausloggen warten zu müssen.
- [x] **Entfernen-Button in Freundeslisten** (Launcher `FriendsList.jsx` + App `FriendsScreen.jsx`): Klick → Inline-Bestätigung ("Wirklich?"/"Abbrechen") → `removeFriend(uuid)`. Lokal sofort ausgeblendet (unabhängig vom Sync-Timing), damit der entfernte Freund nicht durch einen Zwischen-Refresh wieder auftaucht, bevor MMOCore es übernommen hat.
- [x] Rust (`cargo check`), Laravel (`php -l` + Live-Test aller neuen Endpunkte/Commands) und Mobile-Syntax fehlerfrei.
- [ ] **Noch zu tun:** echter Live-Test von "Entfernen" + "Annehmen während online" mit zwei Spielern (Backend/Plugin sind bereit, aber noch nicht durchgeklickt).

### Noch offen aus dem Nutzerwunsch vom 22.07.2026 (nach Teil 4 dran)
- [ ] Profilbild vor dem Namen in Freundeslisten — **neues Konzept an der echten Account-UUID** (`players.uuid`), NICHT am MMOProfiles-Charakterprofil (bestehendes `ProfileController::updatePhoto` ist dafür falsch verdrahtet, muss für Launcher UND App korrigiert werden, siehe Nutzer-Klarstellung: MMOProfiles bleibt für Spielstände/Statistiken, Personalisierung gehört an den Account)
- [ ] Discord-artiges Profil-Popup bei Klick auf einen Freundesnamen (Details + Entfernen-Button darin)
- [ ] Freundes-Chat, Zugriff über Chat-Button im Popup — **Realtime mit Laravel Reverb** (Nutzerentscheidung), größter verbleibender Teil
